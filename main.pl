...
a_before_b(A, B):- format("~a doesn't before ~a", [A, B]), fail.

a_after_b(A, B):- a_before_b(B, A).

a_meets_b(A, B):- (end(A, _a_e), start(B, _b_s), _a_e =:= _b_s), !, format("~a meets ~a", [A, B]).
a_meets_b(A, B):- format("~a doesn't meets ~a", [A, B]), fail.
...
a_during_b(A, B):- format("~a doesn't during ~a", [A, B]), fail.
...
a_equal_b(A, B):- format("~a doesn't equal ~a", [A, B]), fail.
